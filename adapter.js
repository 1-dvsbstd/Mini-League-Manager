...
(function(root){
  ...
  function adapt(data,key,history){
    ...
    if(history){
      ...
      const fullSeason=history.scope==='season';
      if(history.scope && !fullSeason)throw new Error('Unknown history scope.');
      const expectedFirst=current?(fullSeason?1:Math.min(current,...managers.map(m=>number(m.startGw)||1))):null;
      ...
      gameweeks=[...history.gameweeks].sort((a,b)=>a.gameweek-b.gameweek).map(gw=>{
        ...
        const rows=managers.map(m=>{
          const row=byId.get(m.managerId);
          const notStarted=gw.gameweek<(number(fullSeason?row?.teamStartGw:m.startGw)||1)||gw.isUpcoming===true;
          const competitionStarted=gw.gameweek>=(number(m.startGw)||1)&&!notStarted;
          const netPoints=notStarted||row?.dataAvailable===false?null:number(row?.netPoints);
          if(competitionStarted && netPoints===null)cumulativeComplete=false;
          if(competitionStarted&&netPoints!==null)totals.set(m.managerId,(totals.get(m.managerId)||0)+netPoints);
          return {...m,...row,managerId:m.managerId,managerName:m.managerName,teamName:m.teamName,netPoints,notStarted,competitionStarted,transfersMade:number(row?.transfers),hits:number(row?.hitCost),activeChip:({'3xc':'tc','bboost':'bb','freehit':'fh','wildcard':'wc'})[row?.activeChip]||null};
        });
        const weeklyComplete=gw.complete===true&&rows.every(m=>m.notStarted||m.netPoints!==null);
        const seasonRanks=new Map(rank(rows.map(m=>({...m,total:m.competitionStarted?totals.get(m.managerId):null})),'total').map(m=>[m.managerId,m.place]));
    ...
    return {leagueKey:key,seasonKey:data.season.key,updated:data.dataUpdatedAt,
      notice:`${data.season.label||data.season.key}${data.season.isCurrent===false?' · Archived':''} · ${managers.every(m=>m.notStarted)?'This competition starts scoring at GW'+Math.min(...managers.map(m=>Number(m.startGw)))+'. ':''}${data.coverage.complete===false?'Some scores are incomplete. ':''}${history?.scope==='season'?'Season history from GW1. Form and records include games before the paid competition. ':history?'Gameweek history connected. ':'Full gameweek history is not connected yet. '}${history?.gameweeks.some(g=>g.complete!==true)?'Some history scores are unavailable. ':''}${data.capabilities?.squad===1?'Squads load when you open a manager card. ':'Squads are not connected yet. '}Season records update from the available gameweek history.`,
      gameweeks,monthly,
